import { DocPage, LinkType } from '@/lib/db-types'

const icons: DocPage = {
  title: 'Iconos',
  description: ['Iconos para usar en tus proyectos.'],
  contributors: [
    { github_username: 'nsdonato' },
    { github_username: 'IvanMJs' },
    { github_username: 'isahernz' },
  ],
  pageItems: [
    {
      imgPlaceholder: 'Phosphoricons',
      titleCard: '',
      links: [{ type: LinkType.Web, url: 'https://phosphoricons.com/' }],
      cover: {
        src: '/frontend/phosphoricons.svg',
        height: 90,
        width: 80,
      },
      videos: [],
    },
    {
      imgPlaceholder: 'React Symbols',
      titleCard: '',
      links: [{ type: LinkType.Web, url: 'https://react-symbols.vercel.app' }],
      cover: {
        src: '/frontend/reactsymbol.svg',
        height: 90,
        width: 80,
      },
      videos: [],
    },
    {
      imgPlaceholder: 'Feather Icons',
      titleCard: '',
      links: [{ type: LinkType.Web, url: 'https://feathericons.com/' }],
      cover: {
        src: '/frontend/feather.svg',
        height: 90,
        width: 80,
      },
      videos: [],
    },
    {
      imgPlaceholder: 'Hero Icons',
      titleCard: '',
      links: [{ type: LinkType.Web, url: 'https://heroicons.com/' }],
      cover: {
        src: '/frontend/heroicon.svg',
        height: 90,
        width: 80,
      },
      videos: [],
    },
    {
      imgPlaceholder: 'Tabler Icons',
      titleCard: '',
      links: [{ type: LinkType.Web, url: 'https://tabler.io/icons' }],
      cover: {
        src: '/frontend/tabler.svg',
        height: 90,
        width: 80,
      },
      videos: [],
    },
    {
      imgPlaceholder: 'Sargami Icons',
      titleCard: '',
      links: [{ type: LinkType.Web, url: 'https://sargamicons.com/' }],
      cover: {
        src: '/frontend/sargamicons.svg',
        height: 90,
        width: 80,
      },
      videos: [],
    },
    {
      imgPlaceholder: 'Iconoir',
      titleCard: '',
      links: [{ type: LinkType.Web, url: 'https://iconoir.com/' }],
      cover: {
        src: '/frontend/iconoir.svg',
        height: 90,
        width: 80,
      },
      videos: [],
    },
    {
      imgPlaceholder: 'Lucide',
      titleCard: '',
      links: [{ type: LinkType.Web, url: 'https://lucide.dev/' }],
      cover: {
        src: '/frontend/lucide.svg',
        height: 90,
        width: 80,
      },
      videos: [],
    },
    {
      imgPlaceholder: 'Iconbuddy',
      titleCard: '',
      links: [{ type: LinkType.Web, url: 'https://iconbuddy.app/' }],
      cover: {
        src: '/frontend/iconbuddy.svg',
        height: 80,
        width: 80,
      },
      videos: [],
    },
    {
      imgPlaceholder: 'svgl',
      titleCard: '',
      links: [{ type: LinkType.Web, url: 'https://svgl.vercel.app/' }],
      cover: {
        src: '/frontend/svgl.svg',
        height: 80,
        width: 80,
      },
      videos: [],
    },
    {
      imgPlaceholder: 'yesicon',
      titleCard: '',
      links: [{ type: LinkType.Web, url: 'https://yesicon.app/' }],
      cover: {
        src: '/frontend/yesicon.svg',
        height: 80,
        width: 80,
      },
      videos: [],
    },
    {
      imgPlaceholder: 'Remix Icon',
      titleCard: '',
      links: [{ type: LinkType.Web, url: 'https://remixicon.com/' }],
      cover: {
        src: '/frontend/remixicon.svg',
        height: 80,
        width: 80,
      },
      videos: [],
    },
    {
      imgPlaceholder: 'MingCute Icon',
      titleCard: '',
      links: [{ type: LinkType.Web, url: 'https://www.mingcute.com/' }],
      cover: {
        src: '/frontend/mingcuteicon.svg',
        height: 80,
        width: 80,
      },
      videos: [],
    },
    {
      imgPlaceholder: 'Animated Icons',
      titleCard: '',
      links: [{ type: LinkType.Web, url: 'https://animatedicons.co/' }],
      cover: {
        src: '/frontend/animatedicons.svg',
        height: 80,
        width: 80,
      },
      videos: [],
    },
  ],
}

export default icons
